...
Siema.prototype.initPagination = function () {
	this.dots = Array.from(this.selector.parentNode.querySelectorAll('[data-dot]'));
...
Siema.prototype.initControls = function () {
	this.controls = Array.from(this.selector.parentNode.querySelectorAll('[data-control]'));
	...
	if (this.controls.length > 0) {
		this.controls.forEach(control => {
			...
			switch (action) {
				...
				default:
					throw new Error('Unhandled click action provided in slider.js');
...
Siema.prototype.initAutoplay = function (interval) {
	this.autoslide = new timing.Interval(() => { this.next() }, interval, false);

	this.autoslide.start()

	this.selector.parentElement.addEventListener('mouseenter', () => { this.autoslide.stop() });
	this.selector.parentElement.addEventListener('touchstart', () => { this.autoslide.stop() });
	this.selector.parentElement.addEventListener('mouseleave', () => { this.autoslide.start() });
	this.selector.parentElement.addEventListener('touchend', () => { this.autoslide.start() });
}
